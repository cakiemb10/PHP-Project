<?php
/*========================================================*/
// ファイル名：lw_nearkyoten.cgi
// 処理名：【ローソン向けカスタマイズ】最寄り拠点検索
//
// 更新履歴
...
if ($SNDIST != '' && $EWDIST != '') {
	$NELAT = $LAT_DEC + $SNDIST;
	$NELON = $LON_DEC + $EWDIST;
	$SWLAT = $LAT_DEC - $SNDIST;
	$SWLON = $LON_DEC - $EWDIST;
	$res = cnv_ll2lldms($NELAT, $NELON, $DATUM, &$NELAT, &$NELON);
	if (!$res) {
		// 緯度経度不正
		$status = DEF_RETCD_PERR2;
...
$sql .= ", floor( sqrt( power( abs( LAT - $LAT ) * (  9 / 300000 * 1000 ), 2 )";

<?php
/*========================================================*/
// ファイル名：zskr_jurisdictionlist.cgi
// 処理名：信用組合管轄エリアリスト検索
//
// 作成日：2016/02/29
// 作成者：H.Yasunaga
...
/*========================================================*/
	...
	$retdata;
	$retcd = select_union_jurisdiction(&$dba, $union_id, &$retdata);
	if ($retcd != 0) {
		$dba->close();
		if ($retcd == 1) {
	...
	foreach($retdata as $rowdata){
		$buf =  $rowdata["PREFECTURES_CODE"] . "\t" .	// 都道府県コード
				$rowdata["CITY_CODE"]        . "\t" .	// 市区町村コード
				$rowdata["DISPLAY_NAME"]     . "\t" .	// 表示市区町村名
				$rowdata["BC_DISPLAY_NAME"];			// 業態名
		$buf .= "\n";
		print $buf;
	}
